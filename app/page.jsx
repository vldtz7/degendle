...
// Block letter definitions for hint display
const asciiLetters = {
  'A': ["  ██  ", " ████ ", "██  ██", "██████", "██  ██", "██  ██"],
  'B': ["█████ ", "██  ██", "█████ ", "██  ██", "██  ██", "█████ "],
  'C': [" ████ ", "██  ██", "██    ", "██    ", "██  ██", " ████ "],
...
// Generate ASCII text from string
...
const generateStars = () => {
  const stars = [];
  let id = 0;
  // 6 rows of stars spread across the hint box
  for (let row = 0; row < 6; row++) {
    const y = 10 + row * 15; // 10%, 25%, 40%, 55%, 70%, 85%
    const starsInRow = 12 - row; // More stars at top, fewer at bottom
    for (let i = 0; i < starsInRow; i++) {
      const x = 3 + (i / (starsInRow - 1)) * 94; // Spread across 3% to 97%
      stars.push({
        id: id++,
        x: `${x}%`,
        y: `${y}%`,
...
export default function Degendle() {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      try {
        const { guesses: savedGuesses, gameState: savedGameState, answer: savedAnswer } = JSON.parse(savedState);
  ...
  const saveGameState = useCallback((newGuesses, newGameState, newAnswer = null) => {
    if (!puzzle) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.correct) {
        ...
        setShowStars(true);
        saveGameState(newGuesses, 'won', result.answer);
      } else if (result.gameOver) {
        setGameState('lost');
        setAnswer(result.answer);
        setShowTears(true);
        saveGameState(newGuesses, 'lost', result.answer);
      } else {
  ...
  const getBackgroundColor = () => {
    switch (gameState) {
      case 'lost': return '#e0cdd1';
      case 'won': return '#d0e0cd';
      default: return '#cde0dc';
    }
  };
  ...
  const getHintBoxBackground = () => {
    switch (gameState) {
      case 'lost': return '#f0e8ea';
      case 'won': return '#e8f0e6';
      default: return '#e8f2f0';
    }
  };

  // Dynamic border color
  const getBorderColor = () => gameState === 'lost' ? '#4a2d35' : '#2d4a47';
  
  // Dynamic text color
  const getTextColor = () => gameState === 'lost' ? '#4a2d35' : '#2d4a47';
